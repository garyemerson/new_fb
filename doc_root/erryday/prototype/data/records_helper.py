...
sql_data_file_path = "/home/app/doc_root/erryday/prototype/data/records.sqlite"
cmd_select_from_records = """
select records.id, author, day, text, upload_time_unix_ms, img_ids from records
"""
...
def retrieve_records(where_statement=''):
    cmd_to_execute = cmd_select_from_records + where_statement + cmd_group_by_id_sort
...
def __translate_to_records_object(raw_records_list):
    ...
    for record_tuple in raw_records_list:
        translated_list.append(dict({
            ...
            'images': record_tuple[5].split(",") if record_tuple[5] is not None else None
        }))
