...
function authorSelectChange() {
    ...
    xhr.open("GET", "/erryday/get_draft.cgi");
    xhr.send();
}
...
$(document).ready(
    function() {
        ...
        let date = new Date();
        let hotDate = new Date(date.getTime() - (date.getTimezoneOffset() * 60000 ))
                        .toISOString()
                        .split("T")[0];
        $('#date').val(hotDate);
...
$("#fileInput").change(function() {
    ...
    let i = 0;
    var imgDisplayDiv = document.getElementById("fileOutputDiv");
